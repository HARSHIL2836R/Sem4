...
// Creating shortcut for an integer pair and edge data type
typedef pair<int, int> iPair;
typedef pair<int, iPair> edg;

//Disjoint Union Find Data structure: Used in detecting cycle
struct UnionFind
{
...
{ 
	int V, E;
	vector<edg> redges,bedges; // Red edges, Blue edges
	UnionFind UF;
	vector<vector<int>> adj; // Adjacency matrix
	map<edg,bool> valid; // Used in choosing best blue edge to remove from Spanning Tree
	...
	// Utility function to print graphs
	void print()
	{
		cout<<"---\n";
	...
	// Check whether vertices are connected in graph
...
// To make initial MST giving blue edges more preference
...
// DFS implementation to get edges of cycle
void dfs(Graph &st, vector<int>& parent,int p, int u,vector<bool>& visited, vector<int>& cycle_edges){
	...
	for(int i=0;i<st.adj[u].size(); i++){
		...
		else if(v != p){ // extract cycle using parent pointers.
...
edg get_bedge(Graph &st, edg edge)
{
	...
	for (int i = st.bedges.size() - 1; i >= 0; i--)	
	{
		...
		{
			// Checking if the edge is present in cycle
...
// Precompute the blue edges such that replacing them reduces the weight most
...
// Use precomputed values to swap edges, and make suitable modifications
void swap_blue_with_red(Graph &st, priority_queue<pair<int,pair<edg,edg>>> &max_diff)
{
	auto top = max_diff.top(); // Most suitable edges
	max_diff.pop();
	...
	{// modify swaps, blue edge not present
	...
	auto cycle_edges = get_cycle(st); // Verify that no cycle remains
	if (cycle_edges.size() > 0)
...
{ 
	...
	if (wt > threshold)
	{// Swap edges until weight is bounded by threshold
